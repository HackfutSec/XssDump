...
banner = """\033[0;32m
          ...
          [Warning] I am not responsible for the way you will use this program [Warning]
                    ...
                    Description:
                    This Python script is an XSS (Cross-Site Scripting) vulnerability scanner. 
                    It loads payloads from a JSON file, modifies URLs to test vulnerable parameters, 
                    and sends HTTP requests to detect potential XSS issues by checking for the presence of malicious scripts in the response body. 
                    The script uses multithreading for faster testing of multiple URLs in parallel. Results are saved in XSS.txt, and errors are logged in errors.log.
"""
...
# Test XSS
def test_xss(urls, payloads, output_file, option, max_workers=10):
    futures = []
...
def check_vulnerability(url, output_file):
    try:
        ...
        if response.status_code == 200 and ("<script>" in response.text or payload in response.text):
            with file_lock:
...
def main():
    ...
    payloads_file = 'xss_payloads.json'
    output_file = 'XSS.txt'
    ...
    test_xss(urls, payloads, output_file, option, max_workers)
